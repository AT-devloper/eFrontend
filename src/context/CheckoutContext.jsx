...
const Checkout = () => {
  ...
  const verifyPayment = async (id) => {
    setStatusMessage("💳 Verifying payment with backend...");
    try {
      const res = await fetch(`http://localhost:8080/auth/payment/status/check/${id}`);
      const data = await res.json();

      if (data.status === "NOT_FOUND") {
        setStatusMessage("❌ Payment not found. Please contact support.");
      } else if (data.status === "DUE" || data.status === "PENDING") {
        setStatusMessage("⚠️ Payment pending. Please wait a few moments.");
      } else {
        setStatusMessage(`✅ Payment confirmed! Status: ${data.status}`);
      }
    } catch (err) {
      console.error(err);
      setStatusMessage("❌ Could not verify payment. Try again.");
    }
  };
  ...
  const handlePayNow = async () => {
    ...
    const options = {
      ...
      handler: function (response) {
        // 🔹 Verify payment after Razorpay popup
        verifyPayment(response.razorpay_order_id);
      },
      modal: {
        ondismiss: () => setStatusMessage("⚠️ Payment popup closed."),
      },
    };
  ...
  const handlePayMonthly = async () => {
    ...
    const options = {
      ...
      theme: { color: "#4A2E2E" },
      handler: function (response) {
        // 🔹 Verify subscription after Razorpay popup
        verifyPayment(response.razorpay_payment_id || subscriptionId);
      },
      modal: {
        ondismiss: () => setStatusMessage("⚠️ Subscription popup closed."),
      },
    };
              ...
              <Grid container justifyContent="space-between" alignItems="center">
                ...
                <Stack direction="row" spacing={1}>
                  <Button
                    ...
                    sx={{ textTransform: "none", minWidth: 100, py: 0.5, fontWeight: 600 }}
                  >
